...
fn handle_subdivision(
    board: &Box<Board>,
    moves: &Vec<Move>,
    results: &Arc<Mutex<Vec<Vec<Move>>>>,
    moves_iter: &mut Iter<'_, Move>,
    nb_moves: usize,
) {
    loop {
        // can read stale value but this will be caught by the compare_exchange, Relaxed is fine
        ...
        // no data needs to be visible between threads except for the atomics, Relaxed is fine
        if let Ok(old_value) = TASK_COUNT.compare_exchange_weak(
            old_task_count,
            new_count,
            Ordering::Relaxed,
            Ordering::Relaxed,
...
fn main_game() {
    ...
    rayon::spawn(move || {
        game_move(Box::new(board), moves, results_ref);
    });
