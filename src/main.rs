...
fn handle_subdivision(board: &Box<Board>, moves: &Vec<Move>, results: &Arc<Mutex<Vec<Vec<Move>>>>, moves_iter: &mut Iter<'_, Move>, nb_moves: usize) {
    if let Ok(old_task_count) = TASK_COUNT.fetch_update(Ordering::Relaxed, Ordering::Relaxed, |x| {
        let new_task_count = x + std::cmp::min(nb_moves as i32, MAX_TASKS - x);
        Some(new_task_count)
    }) {
